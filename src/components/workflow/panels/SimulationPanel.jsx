...
const CONTENT_TYPES = [
  ...
  { id: 'email',      label: 'Participant enrollment email',       icon: IconMail,         color: 'orange' },
  { id: 'cta',        label: 'Portal banner CTA card',             icon: IconDeviceMobile, color: 'cyan'   },
  { id: 'script',     label: 'Committee deck / plan brief',        icon: IconPhone,        color: 'green'  },
...
const SEG_DETAIL = [
  {
    ...
    path: ['Committee approval of default design', 'Required QDIA / auto-enroll notice', 'Recordkeeping default election staged', 'Opt-out window activated before first deferral'],
    avoid: ['Enrollment without the required advance notice', 'Removing or burying the opt-out path'],
  },
  {
    id: 'seg-2', color: 'yellow',
    label: 'New Hires', count: 7900,
    signal: 'Hire date within eligibility window; no active deferral election yet',
    offer: 'Day-one Auto Enrollment + Escalation',
    channel: 'Onboarding packet + portal banner',
    variantA: '"Your plan starts on day one"',
    variantB: '"Enrolled automatically — here\'s how it grows"',
    why: 'Default enrollment at onboarding sets a durable participation habit and captures employees before inertia sets in.',
    path: ['Onboarding default election set', 'Notice packet delivered', 'Portal banner enabled', 'Escalation schedule attached'],
    avoid: ['Complex opt-in forms at onboarding', 'Omitting escalation disclosure'],
  },
  {
    id: 'seg-3', color: 'blue',
    label: 'Below-Match Participants', count: 6200,
    signal: 'Active deferral below the full-match threshold on payroll feed',
    offer: 'Match Stretch + full-match nudge',
    channel: 'Participant email + portal banner',
    variantA: '"Capture your full employer match"',
    variantB: '"You\'re leaving match on the table"',
    why: 'Below-match savers are already participating; a cost-neutral match stretch plus a one-click increase captures unclaimed employer match.',
    path: ['Match-stretch design modeled', 'Participant email with one-click increase', 'Portal banner reinforcement', 'Payroll election updated'],
    avoid: ['Framing that implies guaranteed returns', 'Match changes without cost modeling'],
  },
  {
    id: 'seg-4', color: 'indigo',
    label: 'Low-Deferral Participants', count: 5800,
    signal: 'Deferral at/near plan minimum; below target-replacement trajectory',
    offer: 'Auto Escalation (1%/yr to 10% cap)',
    channel: 'Portal banner + payroll nudge',
    variantA: '"Grow your savings 1% a year — automatically"',
    variantB: '"Small steps toward your retirement target"',
    why: 'Automatic, gradual escalation lifts deferrals toward readiness while preserving opt-out, with minimal perceived paycheck impact.',
    path: ['Escalation election staged', 'Opt-out notice delivered', 'Payroll nudge enabled', 'Annual increase scheduled'],
    avoid: ['Escalation without an opt-out each cycle', 'Caps above plan-approved limits'],
  },
  {
    id: 'seg-5', color: 'teal',
    label: 'Legacy Election Holders', count: 4700,
    signal: 'Election predates current plan design / QDIA; no update on record',
    offer: 'Re-enrollment into current QDIA',
    channel: 'Required notices + participant email',
    variantA: '"Your investments, brought up to date"',
    variantB: '"A refreshed default — or keep your choices"',
    why: 'Re-enrollment sweeps outdated elections into the current QDIA with opt-out, improving default-fund adoption while respecting existing choices.',
    path: ['Re-enrollment notice mailed', 'QDIA mapping staged', 'Opt-out window activated', 'Confirmation of retained elections honored'],
    avoid: ['Mapping without the right to keep existing elections', 'Insufficient notice period'],
  },
  {
    id: 'seg-6', color: 'green',
    label: 'Near-Retirement Under-Savers', count: 3400,
    signal: 'Age 50+; deferral below catch-up threshold (eligibility-based)',
    offer: 'Catch-up eligibility + escalation education',
    channel: 'Participant email + committee-approved outreach',
    variantA: '"You may be eligible to save more"',
    variantB: '"Catch-up contributions before retirement"',
    why: 'Age-50+ employees below catch-up levels have a short window to improve readiness; education-first framing with escalation is appropriate and welcomed.',
    path: ['Catch-up eligibility surfaced', 'Escalation education module', 'Participant email triggered', 'Optional deferral increase CTA'],
    avoid: ['Individualized advice without eligibility clearance', 'Urgency or fear-based framing'],
  },
  {
    id: 'seg-7', color: 'grape',
    label: 'Auto-Enroll Opt-Outs', count: 2200,
    signal: 'Prior opt-out flag on record; still eligible and non-participating',
    offer: 'Neutral re-engagement at annual window',
    channel: 'Portal + committee review',
    variantA: '"An easy way back in, whenever you\'re ready"',
    variantB: '"Reconsider enrolling this year"',
    why: 'Prior opt-outs must be respected; a neutral re-enrollment offer only at the annual window recovers some participation without pressure.',
    path: ['Suppressed until annual re-enrollment window', 'Neutral offer staged', 'Committee-reviewed timing', 'Opt-out preserved'],
    avoid: ['Repeated outreach outside the annual window', 'Pressuring employees who opted out'],
  },
]
  ...
  return (
    <Stack gap="md">
      ...
      <Paper withBorder p="md" radius="md" style={{ background: 'var(--mantine-color-violet-light)', borderLeft: '3px solid var(--mantine-color-violet-5)' }}>
        <Stack gap="xs">
          ...
          <Text size="xs" c="dimmed" style={{ lineHeight: 1.6 }}>
            TwinX has divided the eligible workforce into cohorts using enrollment status, deferral level, match utilization, tenure, age-based eligibility, and legacy-election status. Cohorts are based on recordkeeping and payroll facts and plan eligibility — reaction-based signals only, at the cohort level, with no individual participant scoring.
          </Text>
        </Stack>
      </Paper>
      ...
      <Alert icon={<IconAlertTriangle size={16} stroke={1.5} />} color="yellow" variant="light">
        <Text size="xs">Participation and deferral lift, opt-out, and employer-cost estimates are simulation outputs based on prior plan-design episode data. Actual results depend on workforce fit, notice timing, and execution quality. Holdout design ensures causal attribution.</Text>
      </Alert>
